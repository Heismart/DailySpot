...
import cn.grokit.dailyspot.entity.zhihu.Answer;
...
public class TopAnswerProcess extends PatternProcessor {
    ...
    public MatchOther processPage(Page page) {
        ...
        Map<Long, CopyOnWriteArrayList<Answer>> map = object.getJSONArray("data").stream().filter(o -> {
            ...
            return new Answer()
                    ...
                    .setTitle(title)
                    .setAnswerUrl(
                            StrUtil.format(Zhihu.ANSWER_PAGE_URL, questionId, answerId)
                    );
    ...
    private void initAllRequest(long topicId, double totals) {
        ...
        for (int i = 0; i < pages - 1; i++) {
            ScrapeUtils.addReqyest(StrUtil.format(Zhihu.ANSWER_URL, topicId, offset += 50));
        }
    }
    ...
    private void initAnswerFile(Topic topic) {
        // 初始化文件
        String fileName = StrUtil.format(
                Zhihu.TOPIC_ANSWER_FILE_NAME,
                "answer",
                topic.getTopicName().replace("/", " "),
                topic.getTopicId(),
                "md"
        );
    ...
    private void writeAnswerFile(Long topicId) {
        ...
        try {
            ...
            // 文件
            String fileName = StrUtil.format(Zhihu.TOPIC_ANSWER_FILE_NAME,
                    "answer",
                    topic.getTopicName().replace("/", " "),
                    topic.getTopicId(),
                    "md"
            );
            String jsonFileName = StrUtil.format(
                    Zhihu.TOPIC_ANSWER_FILE_NAME,
                    "answerJson", topic.getTopicName().replace("/", " "),
                    topic.getTopicId(),
                    "json"
            );
            ...
            map.forEach((qid, answers) -> {
                ...
                answers.forEach(answer -> builder.append(
                                StrUtil.format(
                                        "- [%s的回答](%s),点赞数：%d，评论数：%d\n",
            ...
            List<Long> qids = map.keySet().stream().collect(Collectors.toList());
